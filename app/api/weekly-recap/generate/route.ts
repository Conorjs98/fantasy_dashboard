...
import { buildRecapMatchups } from "@/lib/recap-builder";
...
export async function POST(request: Request) {
  try {
    ...
    const matchups = await buildRecapMatchups(context, week);

    if (matchups.length === 0) {
      return NextResponse.json({ error: "No matchups found for this week" }, { status: 404 });
    }

    const result = await generateRecap(week, context.league.season, matchups, personalityNotes);
