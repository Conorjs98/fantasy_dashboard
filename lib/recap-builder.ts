...
export function buildHighlights(
  ...
  const byMarginAscending = [...matchups].sort(
    (left, right) =>
      Math.abs(left.a.score - left.b.score) -
      Math.abs(right.a.score - right.b.score)
  );
  ...
  const highScore = [...participants].sort(
    (left, right) => right.player.score - left.player.score
  )[0];
  const lowScore = [...participants].sort(
    (left, right) => left.player.score - right.player.score
  )[0];
  ...
  const gameLoser =
    gameWinner.rosterId === gameOfTheWeekMatchup.a.rosterId
      ? gameOfTheWeekMatchup.b
      : gameOfTheWeekMatchup.a;
  const beatdownWinner =
    beatdownMatchup.a.score >= beatdownMatchup.b.score
      ? beatdownMatchup.a
      : beatdownMatchup.b;
  const beatdownLoser =
    beatdownWinner.rosterId === beatdownMatchup.a.rosterId
      ? beatdownMatchup.b
      : beatdownMatchup.a;
...
export async function buildRecapMatchups(
  ...
  for (const [matchupId, group] of grouped) {
    ...
    const a = toParticipant(
      first.roster_id,
      aScore,
      membersByRosterId.get(first.roster_id)
    );
    const b = toParticipant(
      second.roster_id,
      bScore,
      membersByRosterId.get(second.roster_id)
    );
